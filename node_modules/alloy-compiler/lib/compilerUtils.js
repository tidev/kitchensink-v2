...
// //////// private variables //////////
// /////////////////////////////////////
var alloyUniqueIdPrefix = '__alloyId',
...
_.each(CONST.PLATFORMS, function (p) {
	exports.CONDITION_MAP[p] = require('alloy-utils').platforms[p]['condition'];
});
...
exports.generateNode = function (node, state, defaultId, isTopLevel, isModelOrCollection) {
	...
	var parsersDir = path.join(__dirname, 'parsers');
	...
	if (isLocal && state.parent) {
		args.symbol = state.parent.symbol || args.symbol;
	}

	// Use manually given args.symbol if present
	if (state.args) {
		args.symbol = state.args.symbol || args.symbol;
	}
...
exports.expandRequireNode = function (requireNode, doRecursive) {
	...
	function getViewRequirePath(node) {
		...
		if (!src) {
			...
			if (platform) {
				fullpaths.push(path.join(compilerConfig.dir.views, platform, src));
			}
...
exports.createCompileConfig = function (inputPath, outputPath, alloyConfig, buildLog) {
	...
	_.each(libDirs, function (dir) {
		obj.dir[dir] = path.resolve(path.join(__dirname, '..', dir));
	});
...
function generateConfig(obj) {
	...
	var platform = require('alloy-utils').platforms[alloyConfig.platform].titaniumFolder;
...
exports.parseConfig = function (file, alloyConfig, o) {
	...
	if (alloyConfig) {
		...
		if (alloyConfig.deploytype === 'production' && alloyConfig.target) {
			distType = _.find(CONST.DIST_TYPES, function (obj) {
				return obj.value.indexOf(alloyConfig.target) !== -1;
			});
			if (distType) {
...
exports.validateNodeName = function (node, names) {
	...
	ret = _.find(names, function (name) {
