...
function parse(node, state, args) {
	...
	switch (type) {
		case 'view':
			...
			if (platform) {
				paths.push(path.join(appPath, CONST.DIR.VIEW, platform, src));
			}
	...
	if (xChildren.length) {
		xArgs.children = '__ALLOY_EXPR__--[' + xChildren.join(',') + ']';
	}
	if (state.model) {
		xArgs[CONST.BIND_MODEL_VAR] = '__ALLOY_EXPR__--' + state.model;
	}
